...
public class UserController {
    ...
    public BaseResponse<List<User>> searchList(HttpServletRequest request) {
        ...
        if (loginUser != null) {
            ...
            List<User> userList = (List<User>) valueOperations.get("MobaPartner:user:notLogin");
            if (userList != null) {
                // 打乱并固定第一个用户
                return ResultUtil.success(fixTheFirstUser(userList));
            }
        ...
        try {
            if (loginUser != null) {
                ...
                redisTemplate.opsForValue().set("MobaPartner:user:notLogin", result, 10, TimeUnit.MINUTES);
            }
        } catch (Exception e) {
